	...
	switch ($cms_tag_params['type'])
	{
		...
		case 2:
		{
			...
			if($_db->f('tag_v')==$cms_tag_params['tag'])
			{
				...
				if(is_file("php/".$_db->f('template_file_v')))
				{
					include("php/".$_db->f('template_file_v'));
				}
				$ret='';
			...
			elseif ($_db->f('tag_h')==$cms_tag_params['tag'])
			{
				...
				if(is_file("php/".$_db->f('template_file_h')))
				{
					include("php/".$_db->f('template_file_h'));
				}
				$ret='';
			...
			{
				$ret=include("php/".$cms_tag_params['file_name']);
			}
			if($ret)
			{
				return $ret;
...
<script language="javascript" type="text/javascript">
		...
		theme : "simple"
				...
				$ret.='				
				<textarea id="'.$field_name.'" name="'.$field_name.'" style="width: 260px; height: 200px;">'.htmlspecialchars($default_data).'</textarea>';
		...
		case 3:
		{
			$ret='<textarea id="'.$field_name.'" name="'.$field_name.'" class="exercise-desc">'.htmlspecialchars(strip_tags($default_data)).'</textarea>';
		}break;
	}
	return $ret;
}
